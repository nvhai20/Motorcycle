...
(
    SELECT MAX(hinhanh) 
    FROM tbl_sanpham_mau 
    WHERE tbl_sanpham_mau.id_sanpham = tbl_sanpham.id_sanpham
    LIMIT 1
) AS hinh_anh_mau,
...
tbl_sanpham
LEFT JOIN 
tbl_danhmuc ON tbl_sanpham.id_danhmuc = tbl_danhmuc.id_danhmuc
LEFT JOIN 
tbl_cart_details ON tbl_sanpham.id_sanpham = tbl_cart_details.id_sanpham
LEFT JOIN 
tbl_donhang ON tbl_cart_details.code_cart = tbl_donhang.code_cart AND tbl_donhang.cart_status = 3
WHERE 
DATE(tbl_donhang.ngaymua) BETWEEN '$subdays' AND '$now'
GROUP BY 
...
while ($row = mysqli_fetch_array($query_lietke_sp)) {
    ...
    $sql_tong_soluongmau = "SELECT SUM(soluongmau) AS tong_soluongmau FROM tbl_sanpham_mau WHERE id_sanpham = " . $row['id_sanpham'];
    $query_tong_soluongmau = mysqli_query($mysqli, $sql_tong_soluongmau);
    
    if ($query_tong_soluongmau) {
        $row_tong_soluongmau = mysqli_fetch_assoc($query_tong_soluongmau);
        $tong_soluongmau = $row_tong_soluongmau['tong_soluongmau'];
    
        // Sử dụng tổng soluongmau thay vì soluong
        $soluongcon = $tong_soluongmau - $row['total_quantity'];
    } else {
        // Xử lý khi truy vấn không thành công
        $soluongcon = 0; // hoặc bất kỳ giá trị mặc định nào bạn muốn
    }    $giakhuyenmai = $row['giakm'];
    $giaban = $row['giasp'] - ($row['giasp'] * $giakhuyenmai/100);
    $doanhthu = $row['total_quantity'] * $giaban;
    $lai = $doanhthu - ($row['total_quantity'] * $row['giagoc']);
...
<strong>Tổng lợi nhuận: <?php echo number_format($totalProfit, 0, '.', ','); ?> VNĐ</strong><br>
    ...
    while ($row = mysqli_fetch_array($query_lietke_sp)) {
        ...
        <tr style="text-align: center;">
            ...
            <td><img style="width:100px;max-height:100px" src="modules/quanlysp/uploads/<?php echo $row['hinh_anh_mau']; ?>"></td>

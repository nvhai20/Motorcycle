...
if (isset($_SESSION['id_khachhang'])) {
    ...
    if ($result_check_giohang) {
        ...
        while ($row = mysqli_fetch_assoc($result_check_giohang)) {
            ...
            $check_soluong_query = "SELECT (soluongmau - soluongdaban) AS soluongconlai 
                                    FROM tbl_sanpham_mau 
                                    WHERE id_sanpham = '$id_sanpham' AND id_mau = '$mau'";
            ...
            if ($check_soluong_result) {
                ...
                if ($soluongconlai >= $soluong) {
                    // Thêm chi tiết đơn hàng vào bảng tbl_cart_details
                    $insert_order_details = "INSERT INTO tbl_cart_details (id_sanpham, code_cart, soluongmua, mau, ngaymua) 
                                            VALUES ('$id_sanpham', '$code_order', '$soluong', '$size', '$update_time')";
                    mysqli_query($mysqli, $insert_order_details);

                    // Cập nhật số lượng đã bán trong bảng size_soluong
                    $update_soluongdaban_query = "UPDATE tbl_sanpham_mau 
    ...
    foreach ($_SESSION['cart'] as $key => $value) {
        ...
        if ($check_soluong_result) {
            ...
            if ($soluongconlai >= $soluong) {
                // Thêm chi tiết đơn hàng vào bảng tbl_cart_details
                $insert_order_details = "INSERT INTO tbl_cart_details (id_sanpham, code_cart, soluongmua, mau, ngaymua) 
                                        VALUES ('$id_sanpham', '$code_order', '$soluong', '$mau', '$update_time')";
                ...
                // Cập nhật số lượng đã bán trong bảng mau_soluong
                $update_soluongdaban_query = "UPDATE tbl_sanpham_mau 
                                            SET soluongdaban = soluongdaban + $soluong 
                                            WHERE id_sanpham = '$id_sanpham' AND id_mau = '$mau'";
                mysqli_query($mysqli, $update_soluongdaban_query);
    ...
    $insert_order = "INSERT INTO tbl_donhang ( id_khachhang_temp, code_cart, email, tenkhachhang, dienthoai, diachi, payment_method, ngaymua,cart_status)
                    VALUES ('" . $id_khachhang . "','" . $code_order . "','" . $email . "','" . $hoTen . "','" . $soDienThoai . "','" . $diaChi . "','" . $payment_method . "','" . $update_time . "',0)";
    ...
    unset($_SESSION['cart']);
    header('Location:../../index.php?page=ketqua&code_cart=' . $code_order);
}
?>

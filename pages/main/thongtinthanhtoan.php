<div class="shipping-info">
    ...
    <div class="cart_left">
        <form action="pages/main/thanhtoan.php" method="post" onsubmit="return validatePayment();">
            <?php
            $isLoggedIn = isset($_SESSION['id_khachhang']);
            ...
            if ($isLoggedIn) {
                ...
                $sql_pro = "SELECT * FROM tbl_dangky WHERE tbl_dangky.id_dangky = '$id' LIMIT 1";
...
if (isset($_SESSION['id_dangky'])) {
    ...
    // Truy vấn bảng tbl_giohang để lấy thông tin sản phẩm dựa trên id_dangky
    $sql_giohang = "SELECT giohang.*, sanpham.tensanpham, sanpham.giasp, sanpham.hinhanh, mau.tenmau,sanpham.km
                    FROM tbl_giohang giohang
                    JOIN tbl_sanpham sanpham ON giohang.id_sanpham = sanpham.id_sanpham
                    JOIN tbl_mau ON giohang.tbl_mau = mau.id_mau
                    WHERE giohang.id_dangky = $id_dangky";
    ...
    while ($cart_item = mysqli_fetch_assoc($query_giohang)) {
        ...
        <div class="cart-itemss">
            ...
            <div class='spgiohang_oo'>
                ...
                <p>Màu sắc: <?php echo $cart_item['tenmau']; ?></p>
            </div>
    ...
    <?php foreach ($_SESSION['cart'] as $cart_item): ?>
        ...
        $soluongsanpham += $cart_item['soluong'];
        $i++;
        $sql = "SELECT tenmau FROM tbl_mau WHERE id_mau = " . $cart_item['mau'];
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $tenmau = "Kích thước không hợp lệ";
        }
        ?>
        <!-- Thay thế phần Chi tiết giỏ hàng -->
        <div class="cart-itemss">
